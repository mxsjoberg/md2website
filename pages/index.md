...
This journal contains my unstructured and unfiltered notes on different topics. Basically learning about stuff in public. I might consolidate parts as [/notes](/notes.html) or [/posts](/posts.html).

This website was built by one of my [projects](/work.html). You can check it out here: [github.com/mxsjoberg/md2website](https://github.com/mxsjoberg/md2website)
...
- Check this resource on CLI applications using Rust: [rust-cli.github.io/book/index.html](https://rust-cli.github.io/book/index.html). I should use this and [doc.rust-lang.org/book/](https://doc.rust-lang.org/book/) to refactor [github.com/mxsjoberg/pas2wat](https://github.com/mxsjoberg/pas2wat).

    **I created a branch for old code (subset-of-pascal) since moving main to PL/0 with my own modifications (see ebnf-file).**

- How to implement an interactive language? execute when typing (predicting)? Research this more.

    **I have started working on this in this repo: [github.com/mxsjoberg/pascal-in-ruby](https://github.com/mxsjoberg/pascal-in-ruby)**

## Compiler runtimes (interpreters)

Note on class-based OOP vs prototype-based OOP (instances are environments):
...
An AST interpreter interprets AST directly (tree walker, visitor):
...
A bytecode interpreter (VM) translates AST to bytecode then interprets bytecode.

A stack-based VM is stack of operands and operators with result is on top of stack (no registers)
...
A register-based VM is virtual registers with result in accumulator register (virtual registers are mapped to real registers via register allocation)
...
Note on terminology: compiler frontend refers to everything before code generation (parsing, type checking, AST transformations). Compiler backend refers to code generation, IR optimizations, and target-specific optimizations. LLVM is a compiler backend and consider LLVM IR generation part of front-end (need to translate AST to LLVM IR before using LLVM tools).
...
LLVM is a CPU-focused backend used by `clang` and `rustc`, see [code generation](https://rustc-dev-guide.rust-lang.org/backend/codegen.html).

LLVM input is LLVM IR [[AST to TAC to LLVM](ast-to-tac-to-llvm.html)]. LLVM IR is basically annotated assembly.

MLIR have IRs at different abstraction levels (called dialects):
...
A polyhedral optimization is a loop transformation that maps a loop nest to another loop nest (a loop nest is another word for nested loops):

- MLIR `linalg` dialect can be used for tiling optimizations (data locality, cache utilization)

### MLIR operations

In MLIR, `@` is used to denote a function (operation) and `%` is used to denote a variable (value). All values are typed. `func` is dialect for function abstractions, see [func](https://mlir.llvm.org/docs/Dialects/Func/).
...
In the expression `math.ctlz`, `math` is a dialect (see [math](https://mlir.llvm.org/docs/Dialects/MathOps/)) and `ctlz` is an operation to count leading zeros (see [math.ctlz](https://mlir.llvm.org/docs/Dialects/MathOps/#mathctlz-mathcountleadingzerosop)):
...
**TVM is a mess, probably safe to ignore for now**

Example `mm_relu` [[Numpy Linear Relu](numpy-linear-relu.html)] implemented in `tvm.script` (TensorIR):
...
Combining linear and relu to a single tensor function (linear-relu):
...
Linear, add, relu, softmax often implemented in low-level languages (C/C++) or assembly and can be hardware-specific to utilize parallelism.
...
Paper: [The Deep Learning Compiler: A Comprehensive Survey](https://arxiv.org/pdf/2002.03794.pdf)

DL compiler "front-end":

- parsing model (ONNX, pytorch, tensorflow)
...
- auto-tuning
    - find best kernel/ kernel parameters (e.g., using genetic algorithms, or reinforcement learning)
    - tvm [GA tuner](https://github.com/apache/tvm/blob/main/python/tvm/autotvm/tuner/ga_tuner.py)
- code generation
...
[Abstract interpretation](https://en.wikipedia.org/wiki/Abstract_interpretation) is generalized answers to questions without precise answers
...
[Kolmogorov complexity](https://en.wikipedia.org/wiki/Kolmogorov_complexity) is length of shortest program that outputs some string

- `abababababababababababababababab` can be produced by `print("ab" * 16)`
- `4c1j5b2p0cv4w1x8rx2y39umgw5q85s7` can only be produced by `print("".join(random.choices(string.ascii_lowercase + string.digits, k=32)))` or even shorter `print("4c1j5b2p0cv4w1x8rx2y39umgw5q85s7")`

[Instruction selection](https://en.wikipedia.org/wiki/Instruction_selection) techniques:
